...
export const ConstraintTable: React.FC<ConstraintTableProps> = ({ model, onChange }) => {
  ...
  const handleAddConstraint = () => {
    ...
    const newConstraint = createConstraint(`Restricción ${nextIdx}`, '<=', 10, initialTerms);
  ...
  const handleRemoveConstraint = (constraintId: string) => {
    if (model.constraints.length <= 1) return; // Keep at least one constraint
  ...
  return (
    <div id="constraints-table-card" className="liquid-card rounded-2xl p-4 md:p-5 border border-white/80 shadow-xs flex flex-col gap-3.5">
      {/* Header */}
      ...
      <div className="flex flex-col gap-3">
        {model.constraints.map((c, idx) => (
          <div
            ...
            className="p-3.5 rounded-xl bg-white/70 border border-slate-200/70 flex flex-col gap-2.5 shadow-2xs hover:border-slate-300 transition"
          >
            {/* Top row: Name, Operator, RHS, Delete */}
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                <span className="font-mono text-xs font-semibold text-slate-600 w-5">
                  #{idx + 1}
                </span>
                <input
                  type="text"
                  ...
                  placeholder="Nombre de restricción"
                  className="font-medium text-xs text-slate-800 px-2 py-1 rounded bg-white border border-slate-200 focus:border-blue-500 outline-hidden w-40 sm:w-48"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="font-mono font-bold text-xs text-slate-800 px-2.5 py-1 rounded bg-white border border-slate-200 focus:border-blue-500 outline-hidden cursor-pointer"
                >
                  <option value="<=">≤</option>
                  <option value="=">=</option>
                  <option value=">=">≥</option>
                ...
                <div className="flex items-center gap-1">
                  <span className="text-[11px] text-slate-600 font-medium">RHS:</span>
                  <input
                    type="number"
                    step="any"
                    value={c.rhs}
                    ...
                    className="font-mono font-semibold text-xs text-slate-900 px-2 py-1 rounded bg-white border border-slate-200 focus:border-blue-500 outline-hidden w-20 text-right"
                  />
                </div>
            ...
            <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-slate-100">
              <span className="text-[11px] text-slate-600 font-mono shrink-0">Coeficientes:</span>
              <div className="flex items-center gap-2 flex-wrap">
                {model.variables.map((v) => {
                  ...
                  return (
                    <div
                      key={`c-${c.id}-v-${v.id}`}
                      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg bg-slate-50 border border-slate-200/60 focus-within:border-blue-500"
                    >
                      <input
                        type="number"
                        step="any"
                        ...
                        className="w-12 text-right font-mono text-xs text-slate-800 bg-transparent outline-hidden"
                      />
                      <span className="font-mono text-[11px] font-semibold text-slate-600 pr-1">
                        {v.name}
                      </span>
                    </div>
                  );
            ...
            <div className="text-[11px] font-mono text-blue-800 bg-blue-50/50 px-2.5 py-1 rounded-md border border-blue-100/50 truncate">
